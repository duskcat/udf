...
public class StationDetailInfo extends AbstractGenericUDAFResolver {
    ...
    public static class GenericUDAFMkListEvaluator extends GenericUDAFEvaluator {
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException
        {
            ...
            if (p != null) {
                ...
                myagg.container=mergeMap(myagg.container,StringToMap(parameters[0].toString(),p.toString()));
            }
        }
        ...
        public String terminate(AggregationBuffer agg) throws HiveException {
            MkArrayAggregationBuffer myagg = (MkArrayAggregationBuffer) agg;
            String total=String.valueOf(myagg.container.size());
            Set<String> freqs=new HashSet<String>();
            float scancount=(float)myagg.container.get("scancount");
            myagg.container.remove("scancount");
            int activepoint=0;
            for (String key:myagg.container.keySet())
            {
                float value=(float)myagg.container.get(key);
                if(value/scancount>=0.03)
                    freqs.add(key);
                if(value>0)
                    activepoint+=1;
            }
            return (float)activepoint/(float)myagg.container.size()+":"+freqs.toString();
        }
        ...
        public static Map<String,Integer> StringToMap(String scancount,String jsonStr)
        {
            Map<String,Integer> tras=new HashMap<String, Integer>();
            tras.put("scancount",Integer.valueOf(scancount));
            if(jsonStr!="")
            {
