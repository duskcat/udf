...
public class timeocySingleSta extends AbstractGenericUDAFResolver {
    ...
    public static class GenericUDAFMkListEvaluator extends GenericUDAFEvaluator {
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException
        {
            ...
            if (p != null) {
                ...
                myagg.container.put("time",Integer.valueOf(parameters[0].toString()));
                myagg.container=mergeMap(myagg.container,StringToMap(p.toString()));
            }
        }
        ...
        {
            MkArrayAggregationBuffer myagg = (MkArrayAggregationBuffer) agg;
            Map<String,Float> activeFreq=new HashMap<String, Float>();
            float daycount=(float)myagg.container.get("time");
            myagg.container.remove("time");
            for (String key:myagg.container.keySet())
            {
                float value=(float)myagg.container.get(key);
                activeFreq.put(key,value/daycount);
            }
            return activeFreq.toString();
        }
